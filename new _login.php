...
// TEMP DEBUG START - log-only (no output). Remove after use.
error_reporting(E_ALL);
ini_set('display_errors', '0');
ini_set('display_startup_errors', '0');

// Capture fatal errors on shutdown and save details to a log (no echo)
register_shutdown_function(function() {
    $err = error_get_last();
    if ($err !== null) {
        $msg  = "[" . date('Y-m-d H:i:s') . "] FATAL: {$err['message']} in {$err['file']} on line {$err['line']}\n";
        @file_put_contents(__DIR__ . '/../cpanel_debug_errors.log', $msg, FILE_APPEND);
    }
});

// Convert uncaught exceptions to file-only logs
set_exception_handler(function($ex) {
    $msg = "[" . date('Y-m-d H:i:s') . "] EXCEPTION: " . $ex->getMessage() . " in " . $ex->getFile() . " on line " . $ex->getLine() . "\n";
    @file_put_contents(__DIR__ . '/../cpanel_debug_errors.log', $msg, FILE_APPEND);
    exit;
});
// TEMP DEBUG END

// --- Session cookie params must be set before session_start() to avoid headers already sent ---
